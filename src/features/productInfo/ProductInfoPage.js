...
import {
	RETRIEVE_PAGE_META, SEARCH_PRODUCT_INFO_SAGA, GET_CATEGORY_ATTRIBUTE_SAGA,
	PRODUCT_INFO_QUERY_CITY_SAGA, DYNAMIC_FORM_PREFIX
} from './productinfoState.js';
...
export class ProductInfoPage extends Component {
	...
	__query(pagin, oCondition) {
		...
		return new Promise((res, rej) => {
			dispatch({
				type: SEARCH_PRODUCT_INFO_SAGA, data: { ...oCondition, pageSize: page.pageSize, currentPage: page.current - 1 }, cb: (err, data) => {
					if (err) {
						rej(err);
					} else {
						res(data);
